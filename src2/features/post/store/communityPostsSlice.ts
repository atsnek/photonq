...
import {
  LanguageInput,
  PostWhereInputInputAndWhereInputAndWhereInput_2
...
export const createCommunityPostsSlice: TStoreSlice<TCommunityPostsSlice> = (
  ...
  fetchFeaturedPosts: async silent => {
    ...
    const filters: PostWhereInputInputAndWhereInputAndWhereInput_2 = {};
    const postLanguage = get().communityPosts.postLanguage;
    if (postLanguage) {
      filters.language = {
        equals: asEnumKey(LanguageInput, postLanguage)
      };
    }

    const [posts, postsError] = await sq.query(q => {
      const postComm = q.allTrendingPost({
        pagination: { first: 4 },
        filters
      });
      return (
        postComm?.nodes.map(pn => buildPostPreview(q, pn, currentUser)) ?? []
      );
    });
    ...
    set(
      produce((state: TStoreState) => {
        state.communityPosts.latestPosts = {
          ...
          nextCursor:
            posts.hasNextPage && posts.endCursor ? posts.endCursor : undefined,
          prevCursor:
            posts.hasPreviousPage && posts.startCursor
              ? posts.startCursor
              : undefined,
  ...
  deletePost: async id => {
    ...
    set(
      produce((state: TStoreState) => {
        state.communityPosts.featuredPosts.items =
          state.communityPosts.featuredPosts.items.filter(p => p.id !== id);
        state.communityPosts.latestPosts.items =
          state.communityPosts.latestPosts.items.filter(p => p.id !== id);
        state.communityPosts.searchPosts.items =
          state.communityPosts.searchPosts.items.filter(p => p.id !== id);
      })
    );
    return true;
  }
